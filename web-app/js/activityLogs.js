	...
	function createDomUser(data){
		$("div#user div .logstable").text('');
			...
			for(var i = 0; i < data.length; i++){
				var str="";
				str="You "+data[i].activity+" at "+data[i].timestamp;
...
$("div#cart div .logstable").text('');
	...
	if(data[i].cartname!="null") {
			if(data[i].activity=="Cart Created"){
 				...
 				str = "You created a shared cart"+" - "+data[i].cartname+" at "+data[i].timestamp+".";
				htmlStr=htmlStr+"<li>"+str+"</li>";
			}
			else if (data[i].activity=="Product Added"){

				str = "You added a product"+" - "+data[i].products.name+" to "+data[i].cartname+" at "+data[i].timestamp;
				htmlStr=htmlStr+"<li>"+str+"</li>";
			}

			else if(data[i].activity=="Product Removed") {
				str = "You removed a product -"+data[i].products.name+" from the cart "+data[i].cartname+" at "+data[i].timestamp;
				htmlStr=htmlStr+"<li>"+str+"</li>";
			}

			else if(data[i].activity=="Product Updated"){
				str = "You updated a product -"+data[i].products.name+ " in the cart "+data[i].cartname+" at "+data[i].timestamp;
				htmlStr=htmlStr+"<li>"+str+"</li>";
			}

			else if(data[i].activity=="User Added"){
				str = "You added user(s)- "+data[i].groupusers+ " to the shared cart "+data[i].cartname+" at "+data[i].timestamp;
				htmlStr=htmlStr+"<li>"+str+"</li>";
			}

			else if(data[i].activity=="User Removed"){
				str = "You removed a user "+data[i].groupusers+ "from the shared cart "+data[i].cartname+" at "+data[i].timestamp;
				htmlStr=htmlStr+"<li>"+str+"</li>";
			}

			else if(data[i].activity=="Order Placed"){
				str = "You placed an order - "+data[i].cartname;
				htmlStr=htmlStr+"<li>"+str+"</li>";
			}
	 }
	 else{
		 if (data[i].activity=="Product Added"){
			  str = "You added a product"+ " - "+data[i].products.name +" to your user cart at "+data[i].timestamp;
			 	htmlStr=htmlStr+"<li>"+str+"</li>";
		 }

		 else if(data[i].activity=="Product Removed"){
			  str = "You removed a product -"+data[i].products.name+" from your user cart at "+data[i].timestamp;
				htmlStr=htmlStr+"<li>"+str+"</li>";
		 }

		 else if(data[i].activity=="Product Updated"){
			  str = "You updated a product -"+data[i].products.name+ " in your user cart at "+data[i].timestamp;
				htmlStr=htmlStr+"<li>"+str+"</li>";
		 }

		 else if(data[i].activity=="Order Placed"){
			 str = "You placed an order from your user cart at "+data[i].timestamp;
			 htmlStr=htmlStr+"<li>"+str+"</li>";
		 }
	 }
 }
...
for(var i = 0; i < data.length; i++){
			str="You searched for "+data[i].keyword+" at "+data[i].timestamp;
			htmlStr = htmlStr + "<li>" + str + "</li>";
			console.log(data[i]);
